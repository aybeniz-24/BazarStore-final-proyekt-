...
function OnlineOrderExclusive() {
  ...
  const [selectedItem, setSelectedItem] = useState(null); // Popup içindeki seçilmiş item
  const swiperRef = useRef(null);

  useEffect(() => {
    OnlineOrder().then(data => {
      setOnlineOrderExclusive(data);
      setCounts(data.map(() => 1)); // Başlangıçta tüm miqdarlar için 1
    });
  }, []);
